...
func doSendCheck(fp *os.File) {
	...
	switch errVal {
	case 1:
		fmt.Printf("\nError: One file must be generated by MCPT, the other from a morse sending output capture (not MCPT).\n")
	default:
		os.Exit(0)
	}
}
...
func determineFile(path string) (byte, []byte) {
	...
	if err != nil {
		fmt.Printf("\nError: reading file names <%s>. %v\n", path, err)
		os.Exit(1)
	}

	// see if its the MCPT file
	if len(b) >= 1 && b[len(b)-1] == byte(0x08) { //BS
		// the MCPT file
		return byte('m'), b[:len(b)-1]
	} else {
